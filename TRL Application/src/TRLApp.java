	...
	a: while(true) {
		...
		if(option==1) {
			...
			if(patronID != hold.getPatronID()) {
				...
				System.out.println("[PatronID:"+patronID+"] has hold. Please pay the fine before proceeding.");
		}
				...
				if(selection.equalsIgnoreCase("Y")) {
					...
					d:while(true) {
						...
						if(workerIDstart==workerIDend) {
							...
							System.out.println("WorkID is not correct!!!"
									+ "Please re-enter the WorkerID: ");
							int workerIDend2 = input.nextInt();
							if(workerIDstart==workerIDend2) {
								System.out.println("Session completed.");
								break d;
